...
* [`src/my_gpt_ui/`](src/my_gpt_ui/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/my_gpt_ui/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `my-gpt-ui` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/my_gpt_ui/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns my-gpt-ui.example
  (:require [my-gpt-ui.config :as config])
